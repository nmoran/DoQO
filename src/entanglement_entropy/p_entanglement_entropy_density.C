...
int main( int argc, char **argv ) {
	...
	if ( params.g_A_confs > 0 && params.g_B_confs > 0 ) {
		...
		Mat C;
		Mat WT;
		ierr = MatTranspose(params.W,MAT_INITIAL_MATRIX,&WT);CHKERRQ(ierr);
		ierr = MatMatMult(WT,params.W,MAT_INITIAL_MATRIX,PETSC_DEFAULT,&C);CHKERRQ(ierr);
		...
		ierr = EPSSetOperators(eps,params.W,PETSC_NULL);CHKERRQ(ierr);
		ierr = EPSSetProblemType(eps, EPS_HEP);CHKERRQ(ierr);
		
		PetscInt min_dimension = params.g_A_confs;
		if ( min_dimension <  NSV_LIMIT ) {
			ierr = EPSSetDimensions(eps,min_dimension,min_dimension,min_dimension);CHKERRQ(ierr);
		} else {
			ierr = EPSSetDimensions(eps,NSV_LIMIT,((NSV_LIMIT*3/2)>min_dimension)?min_dimension:(NSV_LIMIT*3/2),NSV_LIMIT);CHKERRQ(ierr);
		}
		...
		if (nconv>0) {
			...
			for( PetscInt i=0; i<nconv; i++ ) {
				...
				PetscScalar sigmai;
				ierr = EPSGetEigenvalue(eps,i,&sigma,&sigmai);CHKERRQ(ierr);
				...
				xi = -PetscLogScalar(PetscAbsComplex(sigma));
				ent_ent = ent_ent -sigma * PetscLogScalar(PetscAbsComplex(sigma));
