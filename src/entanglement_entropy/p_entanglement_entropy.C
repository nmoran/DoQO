#include "p_entanglement_entropy.h"
...
int main( int argc, char **argv ) {
	...
	//see if a rotational symmetry file is specified which indicates the use of rotational symmetry.
	params.rot_symmetry = PETSC_FALSE;
	ierr = PetscOptionsGetString(NULL,"-rot_symmetry",params.rotation_info.filename,MAX_STRING,&flg);CHKERRQ(ierr);
	if ( flg ) {
		PetscPrintf(PETSC_COMM_WORLD,"Rotational symmetry file %s.\n",params.rotation_info.filename);
		params.rot_symmetry = PETSC_TRUE;
		ierr = PetscOptionsGetInt(NULL,"-rot_symmetry_sector",&params.rotation_info.current_sector,&flg);CHKERRQ(ierr);
		if ( flg == PETSC_FALSE ) {
			PetscPrintf(PETSC_COMM_WORLD,"Must specify which rotation section with -rot_symmetry_sector if using rotational symmetry.\n");
			return 1;
		}
		params.rotation_info.real_sector = params.rotation_info.current_sector;
	}	
	...
	if (params.rot_symmetry ) {
		read_rotation_information(&params);
		calculate_rotation_phases(&params);
	}
	
	//just a test of the translation functionality now.
	/*int len = 36;
	uPetscInt test_mask = 0;
	int mask_len = 10;
	for ( int i = len-mask_len ; i < len; i++ ) {
		test_mask += ((uPetscInt)1) << i ;
	}
	
	//int mask_len2 = __builtin_popcount(test_mask);
	int mask_len2 = 0;
	for ( int i = 0 ; i < len ; i++ ) {
		if ( (test_mask & ((uPetscInt)1) << i ) > (uPetscInt) 0 ) {
			mask_len2++;
		}
	}
	PetscPrintf(PETSC_COMM_WORLD,"Mask len: %d\n",mask_len2);
	
	uPetscInt test_mask2 = 0;
	for ( int i = len-2 ; i < len; i++ ) {
		test_mask2 += ((uPetscInt)1) << i ;
	}
	
	dec2bin( test_mask2, len, buf );
	PetscPrintf(PETSC_COMM_WORLD,"Before translating: %s\n",buf);
	
	int count = 0;
	PetscInt *site_indices;
	ierr = PetscMalloc(mask_len2*sizeof(PetscInt),&site_indices);CHKERRQ(ierr);
	int pos = 0;
	while ( count < mask_len2 && pos < len) {
		//		if ( (test_mask & (((uPetscInt)1) << pos) ) > (uPetscInt)0 )  {
		if ( (test_mask & (((uPetscInt)1) << pos) ) > 0 )  {
			site_indices[count++] = pos;
		}
		pos++;
	}
	
	uPetscInt test_mask3 = translate(test_mask2,site_indices,count); 
	dec2bin( test_mask3, len, buf );
	PetscPrintf(PETSC_COMM_WORLD,"After translating: %s %ld\n",buf,test_mask3 );
	PetscFree(site_indices);*/
	//end tests


	params.representative_check = &dummy_basis_check;
	PetscInt max_filling = params.A_filling;
	...
	max_filling = params.filling;
	if ( params.rot_symmetry ) {
		params.representative_check = &rotation_representative_check;
	}
	...
	if ( params.g_A_confs > 0 && params.g_B_confs > 0 ) {
		...
		if ( params.ascii ) {
			ierr = PetscViewerASCIIOpen(PETSC_COMM_WORLD, vec_file.c_str() ,&viewer);CHKERRQ(ierr); //ascii not working well for large sizes at the moment.
		} else {
...
//		PetscInt non_zero_count = 0;
		...
		//		non_zero_count++;
			//}
		//}
		if ( params.rot_symmetry == PETSC_FALSE ) {
			//if we are not using rotation symmetry then the amount of space needed is just the size of the AB confs list.
			ierr = PetscMalloc(params.l_AB_confs*sizeof(class my_vec_entry),&params.my_vec_entries);CHKERRQ(ierr);
		} else {
			//if using rotation symmetry we need to expand this list and it can have a max possible size of the number of sectors times the number in the orig AB confs list. 
			ierr = PetscMalloc(params.rotation_info.num_sectors * params.l_AB_confs*sizeof(class my_vec_entry),&params.my_vec_entries);CHKERRQ(ierr);
		}
		
		PetscInt tmp_idx = 0;
		class my_vec_entry *curr_entry = &params.my_vec_entries[tmp_idx]; //this pointer points to the start of the list
		if ( params.rot_symmetry ) {
			for ( PetscInt idx = 0 ; idx < params.l_AB_confs ; idx++ ) {
				//char buf[MAX_STRING];
				//dec2bin( params.AB_confs[idx] , params.AB_sites, buf );
				//PetscPrintf(PETSC_COMM_WORLD,"%s: %lf + i%lf\n",buf,PetscRealPart(vec_elements[idx]),PetscImaginaryPart(vec_elements[idx]));
				tmp_idx += add_rotationally_related(&params,params.AB_confs[idx],vec_elements[idx],&curr_entry);
			}
			params.l_AB_confs = tmp_idx ; //the new number of confs
		} else {
			for ( PetscInt idx = 0 ; idx < params.l_AB_confs ; idx++ ) {
		...
		sort(params.my_vec_entries, params.my_vec_entries + params.l_AB_confs); //sort according to the b mask.
		
		/*PetscPrintf(PETSC_COMM_WORLD,"\n");
		for ( int j = 0 ; j < params.l_AB_confs ; j++ ) {
			char buf[MAX_STRING];
			dec2bin( params.my_vec_entries[j].conf , params.AB_sites, buf );
			PetscPrintf(PETSC_COMM_WORLD,"%s: %lf + i%lf\n",buf,PetscRealPart(params.my_vec_entries[j].val),PetscImaginaryPart(params.my_vec_entries[j].val));
		}*/
		
		//create boolean value to keep track of whether matrix is all zeros or not. use in for better exchange with MPI functions.
		
		
		
		//now we start the peer to peer communication to exchange vector values. 
		...
		chunk_start = find_start_of_range(params.my_vec_entries,params.l_AB_confs,lower_limit,upper_limit,params.B_mask,&chunk_size); 
		for ( PetscInt idx = chunk_start ; idx < chunk_start + chunk_size ; idx++ ) {
			...
			if ( row >= 0 && col >= 0  ) {
				 MatSetValue(params.W,row+get_rank_chunk_start(params.g_B_confs,params.rank,params.size),col,params.my_vec_entries[idx].val,ADD_VALUES);
			}
		}
		...
		while ( proc_offset < params.size ) { //loop over an iterator so that each process communicates with each other process. 
			...
			if ( recv_chunk_size > 0 ) {
				...
				for ( PetscInt idx = 0 ; idx < recv_chunk_size ; idx++ ) {
					...
					if ( row >= 0 && col >= 0  ) {
						MatSetValue(params.W,row+get_rank_chunk_start(params.g_B_confs,params.rank,params.size),col,recv_buf[idx].val,ADD_VALUES);
					}
				}
				PetscFree(recv_buf);
			}
        ...
        // Commenting out the following check since the MatGetArray and MatRestoreArray are no longer supported.
		//check to make sure there are non zero values.
		/*PetscInt mat_all_zeros = 1;
		PetscScalar *mat_vals;
		ierr = MatGetArray(&params.W,&mat_vals);CHKERRQ(ierr);
		for ( PetscInt idx = 0 ; idx < params.l_B_confs*params.l_A_confs ; idx++ ) {
		    if (PetscAbsScalar(mat_vals[idx]) > 1.0e-10 ) {
			  //PetscPrintf(PETSC_COMM_SELF,"Non zero val: %le + i%le\n",PetscRealPart(mat_vals[idx]),PetscImaginaryPart(mat_vals[idx]));
			  mat_all_zeros = 0;
		    }
		}
		ierr = MatRestoreArray(params.W,&mat_vals);CHKERRQ(ierr);*/
		PetscInt total_zero_check = 1;
		/*MPI_Allreduce(&mat_all_zeros,&total_zero_check, 1, PETSC_MPI_INT, MPI_PROD, PETSC_COMM_WORLD);
		PetscPrintf(PETSC_COMM_SELF,"All zeros. %d\n", mat_all_zeros);*/

		total_zero_check = 0; // faking that we have passed the test
		...
		//check if matrix contains non zeros.
		if ( total_zero_check == 0 ) { 
			SVD svd;
				...
				*/
				...
				for( PetscInt i=0; i<nconv; i++ ) {
					...
					*/
					...
					if ( PetscAbsScalar(error/sigma) < ERROR_MARGIN ) {
	...
	//deallocate space used to store rotation specific info.
	if ( params.rot_symmetry ) {
		for ( int i = 0 ; i < params.rotation_info.num_sectors ; i++ ) {
			PetscFree(params.rotation_info.mapping_masks[i]);
		}
		PetscFree(params.rotation_info.mapping_masks);
		PetscFree(params.rotation_info.phases);
	}
